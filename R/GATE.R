  ...
  for( g in 1:G )
    {
      ...
      num <- pi2.0 * TestStatistic[[g]]$f0x/TestStatistic[[g]]$fx * f.prod - pi2.0^mg * f0.prod
      TestStatistic[[g]]$fdr.j.g <- num/den
      ## Calculate the posterior probability that both theta_g and theta_{j|g}=1
...
GATE.em <- function( TestStatistic, pi1.ini, pi2.1.ini, L, muL.ini, sigmaL.ini, cL.ini, DELTA, sigma.KNOWN=FALSE, a=1, b=2)
## em <- function( TestStatistic, L=2, pi1.ini=0.5, pi2.1.ini=0.5, muL.ini=rnorm(L), sigmaL.ini=rgamma(L, 2, 2), cL.ini=rdirichlet(1, array(1,L), DELTA=0.001 )
{
  ...
  while( delta> DELTA){
    ...
    if( sigma.KNOWN==FALSE ){
      ...
      sigmaL.new <- sqrt( (2*a+ total.sigmaL)/(2*b+total.m.j.g.L) )
    }else{
    ...
    delta <- (pi1.new-pi1.old)^2 + sum( (pi2.1.new-pi2.1.old)^2 ) + sum( (muL.new-muL.old)^2 ) + sum( (cL.new-cL.old)^2 ) + sum( (sigmaL.new-sigmaL.old)^2 )
...
solve.pi2.1.new <- function(Part.A, Part.B, fdr.g.all, mgs )
{
    ...
    for( i in 1:900)
    {
        y0[i] <- Part.A * log( 1-x0[i] ) + Part.B * log( x0[i] ) - sum( log( 1-(1-x0[i])^mgs) * (1-fdr.g.all ) )
    }
    x0[ which(y0==max(y0) ) ]
}
...
## em <- function( TestStatistic, L=2, pi1.ini=0.5, pi2.1.ini=0.5, muL.ini=rnorm(L), sigmaL.ini=rgamma(L, 2, 2), cL.ini=rdirichlet(1, array(1,L), DELTA=0.001 )
{
  ...
  while( delta> DELTA){
    ...
    pi1.old <- pi1.new; pi2.1.old <- pi2.1.new; muL.old <- muL.new; sigmaL.old <- sigmaL.new; cL.old <- cL.new;
    print(delta)
  }## End the while loop for the iteration
...
GATE.4 <- function(TestStatistic, alpha=0.05, eta=alpha, increments=100)
  {
      ...
      if( Sg==0 ){
          ...
          alpha_star = alpha * max.ind/increments
      ...
      list( TestStatistic=TestStatistic, alpha_star = alpha_star )
  }
...
GBH.datadriven <- function(TestStatistic, method="LSL", alpha=0.05)
{
  ...
  if( method=="LSL")
    {
      ...
      for( g in 1:G )
        {
          p.value <- 2*pnorm( -abs( TestStatistic[[g]]$X) )
          mgs[g] <- TestStatistic[[g]]$mg
          ## The following block of code calculate the estimator of pi2.0[g]
          p.value.sort <- sort( p.value, decreasing=FALSE )
          l.g.i <- ( mgs[g] + 1 - c(1:mgs[g]) )/( 1- p.value.sort )
          DIFF <- l.g.i[ 2:mgs[g] ] - l.g.i[1: (mgs[g]-1)]
          IND <- c( 2:(mgs[g]) ) * (DIFF>0)
          if( sum(IND==0) )
            {
              pi2.0[g]=1
              pi2.1[g]=0
            }else{
              pi2.0[g] <- min( (l.g.i[ min( IND[ IND>0 ] ) ]+1/mgs[g]), 1)
              pi2.1[g] <- 1-pi2.0[g]
            }
          ## #####################################################
          ## #####################################################
          if(pi2.1[g]==0){
            p.value <- p.value * 0 + 1000
          }else{
            p.value <- pi2.0[g]/pi2.1[g] * p.value
          }
          TestStatistic[[g]]$weighted.p.value <- p.value ## Store the weighted p.value
          all.weight.p.value <- c(all.weight.p.value, p.value)
        }
      ...
      if( Rej==0){
        for(g in 1:G)
          {
            TestStatistic[[g]]$gbh.lsl.bg.rej <- 0
            TestStatistic[[g]]$gbh.lsl.wg.rej <- ( TestStatistic[[g]]$weighted.p.value)*0
            TestStatistic[[g]]$weighted.p.value <- NULL
          }
      }else{
        thresh <- all.weight.p.value[ Rej ]
        for(g in 1:G)
          {
            TestStatistic[[g]]$gbh.lsl.wg.rej <- ( TestStatistic[[g]]$weighted.p.value <= thresh )
            TestStatistic[[g]]$gbh.lsl.bg.rej <- ( sum(TestStatistic[[g]]$gbh.lsl.wg.rej) > 0 )
            TestStatistic[[g]]$weighted.p.value <- NULL
          }
      }
    }
    ...
    if( method=="TST")
      {
        ...
        for( g in 1:G )
          {
            ...
            ## The following block code calcualte pi2.0 using tst method
            alpha.prime <- alpha/(1+alpha)
            p.value.sort <- sort(p.value, decreasing=FALSE)
            R.group <- max( ( p.value.sort < c(1:mgs[g])*alpha.prime/mgs[g] )* c(1:mgs[g]) )
            pi2.0[g] <- (mgs[g] - R.group)/mgs[g]
            pi2.1[g] <- 1-pi2.0[g]
            ## #######################################################
            ## #######################################################
            if( pi2.1[g]==0)
        ...
        if( Rej==0){
          for(g in 1:G)
            {
              TestStatistic[[g]]$gbh.tst.bg.rej <- 0
              TestStatistic[[g]]$gbh.tst.wg.rej <- ( TestStatistic[[g]]$weighted.p.value)*0
              TestStatistic[[g]]$weighted.p.value <- NULL
            }
        }else{
          thresh <- all.weight.p.value[ Rej ]
          for(g in 1:G)
            {
              TestStatistic[[g]]$gbh.tst.wg.rej <- ( TestStatistic[[g]]$weighted.p.value <= thresh )
              TestStatistic[[g]]$gbh.tst.bg.rej <- ( sum(TestStatistic[[g]]$gbh.tst.wg.rej) > 0 )
              TestStatistic[[g]]$weighted.p.value <- NULL
            }
        }
      }
